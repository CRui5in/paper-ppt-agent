...
def test_direct_import_generates_llm_design_spec_and_rewrites_asset_refs(workspace_tmp: Path, monkeypatch):
    templates_root = workspace_tmp / "assets" / "templates"
    (templates_root / "layouts").mkdir(parents=True)
    (templates_root / "design_spec_reference.md").write_text(
        "# Reference\n\n## I. Project Information\n\n## XI. Technical Constraints Reminder\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(settings, "templates_dir", templates_root)
    import_id = "directspec123"
    import_root = settings.workspaces_dir / "template_imports" / import_id
    pptx_path = import_root / "source.pptx"
    pptx_path.parent.mkdir(parents=True, exist_ok=True)
    pptx_path.write_bytes(b"pptx")
    initialize_import_task(import_id, pptx_path)
    template_importer_module.set_import_collaboration_mode(import_id, "direct")

    work_dir = _work_dir(import_id)
    _write_png(work_dir / "assets" / "logo.png")
    for index in range(1, 6):
        svg_path = work_dir / "svg" / f"slide_{index:02d}.svg"
        svg_path.parent.mkdir(parents=True, exist_ok=True)
        svg_path.write_text(
            """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 720">
  <image href="../assets/logo.png" x="20" y="20" width="80" height="32"/>
  <text x="100" y="120">Direct page</text>
</svg>""",
            encoding="utf-8",
        )
    _write_json(
        work_dir / "manifest.json",
        {
            ...
            "slideSize": {"width_px": 1280, "height_px": 720},
            "theme": {"colors": {"accent1": "#0055AA"}},
            "slides": [{"index": i, "textSamples": [f"Slide {i}"]} for i in range(1, 6)],
        },
    )
    _write_json(
        _review_path(import_id),
        {
            ...
            "slides": [],
            "assets": [],
            "draft": {"label": "Direct Source"},
        },
    )

    async def fake_design_spec(model_config, payload, review):
        assert payload["user_feedback"] == "Generate a complete design_spec.md for this Direct five-slide template."
        assert review["draft"]["page_selections"] == {
            "cover": 1,
            "toc": 2,
            "chapter": 3,
            "content": 4,
    ...
    assert review["draft"]["design_spec"].startswith("# Direct spec")

    result = confirm_direct_import_template(import_id)
    assert result.status == "complete"

    layout_dir = settings.templates_dir / "layouts" / "user_direct_spec"
    assert (layout_dir / "design_spec.md").read_text(encoding="utf-8").startswith("# Direct spec")
    cover_svg = (layout_dir / "01_cover.svg").read_text(encoding="utf-8")
    assert 'href="assets/logo.png"' in cover_svg
    assert "../assets/logo.png" not in cover_svg
    assert "data:image/png;base64," in inline_svg_asset_refs(layout_dir / "01_cover.svg", max_bytes=2_000_000)
